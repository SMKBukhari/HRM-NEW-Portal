...
const formatDate = (d?: Date | null, format?: string): string => {
  if (!d) return "";
          ...
          <FormControl>
            ...
            >
              <InputOTPGroup className='gap-2'>
                {" "}
                {/* Added gap for better visuals */}
                {[...Array(6)].map((_, index) => (
                  <InputOTPSlot
                    key={index}
                    index={index}
                    className={cn(
                      "w-14 h-14 text-lg font-bold", // Standardize size
                      error && "border-red-500 text-red-500", // Error State
                      !error && isFocused && "border-primary" // Focus State (manual override if needed)
                    )}
                  />
                ))}
              </InputOTPGroup>
              ...
              >
                ...
                <span className='truncate'>
                  {current
                    ...
                    : config?.placeholder ?? props.placeholder ?? "Select date"}
                </span>
              </Button>
            </PopoverTrigger>
            <PopoverContent
